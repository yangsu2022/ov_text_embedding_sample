...
class OVBgeEmbeddings(BaseModel, Embeddings):
    ...
    def __init__(self, **kwargs: Any):
        ...
        self.ov_model = core.compile_model(Path(self.model_path), self.model_kwargs["device"], ov_config)
    ...
    def encode(
        ...
        for start_index in trange(0, len(sentences), batch_size, desc="Batches", disable=not show_progress_bar):
            ...
            # print(" out_features[0]:  ", out_features[0])
...
def acc_check(model_name_or_path, ov_model_path, embedding_device):
    if ((Path(ov_model_path)).is_file()):
        print(ov_model_path, " exists")
    else:
        raise Exception(ov_model_path, " not exists")
    print("--- acc_check pytorch vs ov model: ", ov_model_path)
...
def main():
    ...
    # compare ov fp16 int8 int4 on GPU
    acc_check(model_name_or_path, "bge-m3-ov\openvino_model_dyn.xml", 'GPU') # optimum-cli export openvino bge-m3-ov-dyn-int4 --model BAAI/bge-m3 --task feature-extraction --trust-remote-code --framework pt --library sentence_transformers --weight-format int4 --group-size 128 --sym --ratio 1
    acc_check(model_name_or_path, "bge-m3-ov\openvino_model_int8_dyn.xml", 'GPU')
    acc_check(model_name_or_path, "bge-m3-ov-dyn-int4\openvino_model.xml", 'GPU')
    return 0
